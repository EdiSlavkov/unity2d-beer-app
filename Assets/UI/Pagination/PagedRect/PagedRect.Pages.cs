...
namespace UI.Pagination
{
    public partial class PagedRect
    {
        ...
        [SerializeField] private PagedRectContentPivotController centerContent;
        ...
        public void SetClosestPage()
        {
            UpdatePages();
            if (Pages.Count == 1)
            {
                centerContent.ChangePivotToParentPivot();
            }
            else
            {
                centerContent.ResetPivot();
            }
            if (isFirstLoading)
            {
                SetCurrentPage(1);
            ...
            if (Pages.Any(p => p.PageNumber == previousPageNumber))
            {
                SetCurrentPage(GetPageByNumber(previousPageNumber));
                previousPageNumber = currentPageNumber;
                return;
            }
            int closestPageIndex = (int)(Pages.OrderBy(p => p.PageNumber).FirstOrDefault(p => p.PageNumber > currentPageNumber)?.PageNumber ?? 0);
            if (closestPageIndex == 0)
            {
                closestPageIndex = (int)(Pages.OrderByDescending(p => p.PageNumber).FirstOrDefault(p => p.PageNumber < currentPageNumber)?.PageNumber ?? 0);
            }
            if (closestPageIndex != 0)
            {
                SetCurrentPage(GetPageByNumber(closestPageIndex));
            }
            previousPageNumber = currentPageNumber;
            UpdatePages();
        }
